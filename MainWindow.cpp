    ...
    : QMainWindow(parent), currentFile(""), clientId(clientId), LastKnownText(""), charAdded(0)
{
    ...
    // Sync button states with cursor
...
// Keep all the existing methods from both files below this point
// (onConnected, onMessageReceived, onDisconnected, onTextChanged, sendTextMessage, etc.)
// Make sure to include all the methods from both files
